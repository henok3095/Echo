              ...
              <button
                ...
                onClick={async () => {
                  try {
                    ...
                    const { data, error } = await supaAuth.signInWithGoogle()
                    if (error) {
                      toast.error(error?.message || 'Google sign-in failed')
                      setIsOauthRedirecting(false)
                      return
                    }
                    // In browser environments, Supabase will redirect automatically.
                    // If it doesn't (e.g., certain Electron setups), manually follow the URL.
                    if (data?.url) {
                ...
                className="w-full flex items-center justify-center gap-3 border border-gray-300 dark:border-gray-700 rounded-xl py-2.5 text-gray-800 dark:text-gray-100 hover:bg-gray-50 dark:hover:bg-gray-800 transition-colors"
              >
                {/* Google icon */}
                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 48 48" className="w-5 h-5">
                  <path fill="#FFC107" d="M43.611 20.083H42V20H24v8h11.303c-1.649 4.657-6.08 8-11.303 8-6.627 0-12-5.373-12-12s5.373-12 12-12c3.059 0 5.842 1.154 7.961 3.039l5.657-5.657C33.64 6.053 29.084 4 24 4 12.955 4 4 12.955 4 24s8.955 20 20 20 20-8.955 20-20c0-1.341-.138-2.65-.389-3.917z"/>
                  <path fill="#FF3D00" d="M6.306 14.691l6.571 4.819C14.655 16.108 18.961 14 24 14c3.059 0 5.842 1.154 7.961 3.039l5.657-5.657C33.64 6.053 29.084 4 24 4 16.318 4 9.656 8.337 6.306 14.691z"/>
                  <path fill="#4CAF50" d="M24 44c5.166 0 9.86-1.977 13.409-5.192l-6.19-5.238C29.211 35.091 26.715 36 24 36c-5.202 0-9.619-3.317-11.283-7.953l-6.535 5.032C9.5 39.556 16.227 44 24 44z"/>
                  <path fill="#1976D2" d="M43.611 20.083H42V20H24v8h11.303c-.792 2.237-2.231 4.166-3.994 5.571l.003-.002 6.19 5.238C35.271 40.205 40 32 40 24c0-1.341-.138-2.65-.389-3.917z"/>
                </svg>
                <span className="font-semibold">{isOauthRedirecting ? 'Redirecting…' : 'Continue with Google'}</span>
              </button>
